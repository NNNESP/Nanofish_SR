...
```{r select outcome data, echo=FALSE, warning=FALSE}
...
```{r random effects structure, echo=FALSE}
...
t2 <- data.frame(
  Comparison = c(
    ...
    "+ParticleMaterial",
    "+Species"),
  LRT = sapply(res, function(x) round(x$LRT,2)),
...
mRE[[bias]][["pi"]] <- predict(mRE[[bias]], newmods = c(0,mean(df$Year)), digits = 2)
...
if (mRE[[bias]]$pval[1]<0.10){
  ...
  mRE[[mname]][["pi"]] <- predict(mRE[[mname]], newmods = c(0,mean(df$Year)), digits = 2)
  bias<-mname
}

# conditional estimate
# it it is significant there is no longer a single average effect (since the effect depends on the value of the moderator), so if we want to estimate the size of the effect, we have to specify a value for the moderator. One possibility is to estimate the ES based on the average of the moderator of all trials included in the meta-analysis.
```
...
mRE[[bias2]][["pi"]] <- predict(mRE[[bias2]], newmods = c(0,mean(df$Year)), digits = 2)

# PEESE + year
if (mRE[[bias2]]$pval[1]<0.10){
...
if (mRE[["PET sensitivity"]]$pval[1]<0.10){
...
##Leave Embryo or Unclear out + Rho range
```{r}
if (sum(df$DevelopmentalStage %in% c("Embryo", "Unclear"))>0){
  
if (VCVrho != 0){rho_range <- c(0.25, 0.5, 0.75, 0.9)} else {rho_range <-0}

df_temp <- df[!df$DevelopmentalStage %in% c("Embryo", "Unclear"), ]

for (j in 1:length(rho_range)) {
    
    mname <- paste("rho", rho_range[j],"_drop_EmbryoUnclear")
    print(mname)
...
```{r model 1: particle size + duration + developmental stage, echo=TRUE}
...
#are there differences between the different forms of DevelopmentalStage?
#mods_formula <- formula(~ ParticleSizeMean
#                             + ExposureDurationDays*DevelopmentalStage)

#at least one of the DevelopmentalStage levels is different than zero?
morethan1stages <- length(unique(df$DevelopmentalStage))>1
print(c('more than 1 developmental stage: ',morethan1stages))
if (morethan1stages)  {
mods_formula <- formula(~ 0 + ParticleSizeMean
                             + ExposureDurationDays*DevelopmentalStage)
}else{ #for Sensory-motor function
  mods_formula <- formula(~ 0 + ParticleSizeMean
...
if (n_outcome!=10){
anova(mMR[[mname2]], btt=c("DevelopmentalStage","ExposureDurationDays","ParticleSizeMean"))
}else if (n_outcome==10){ #for Sensory-motor function
  anova(mMR[[mname2]], btt=c("ExposureDurationDays","ParticleSizeMean"))
}
m<-mMR[[mname2]]
...
m$fit.stats
```
The meta-regression comprised `r nES_MR` comparisons from `r nEC_MR` control group clusters of `r nSt_MR` independent studies (4lvl).
The meta-regression comprised `r nES_MR` comparisons from `r nEC_MR` independent studies (3lvl).
...
}else if (n_outcome==10){
p1 + plot_spacer() + plot_spacer() + plot_spacer() + p5 + p3 + plot_layout(design = layout)
}
...
```{r model 2: +concentration, echo=TRUE}
morethan3concentrations <- length(unique(df$Concentration_mg_L))>3 #one can be NA
print(c('more than 3 concentrations: ',morethan3concentrations))

if (morethan3concentrations){
...
mods_formula <- formula(~ 0 + ParticleSizeMean
  ...
  VCV <- VCV[!df_MR$DevelopmentalStage %in% c("Adult"),!df_MR$DevelopmentalStage %in% c("Adult") ]
df_MR <- df_MR[!df_MR$DevelopmentalStage %in% c("Adult"), ]
}else{
mods_formula <- formula(~  0 + ParticleSizeMean
                             + Concentration_mg_L
                             + ExposureDurationDays*DevelopmentalStage)
...
The meta-regression comprised `r nES_MR` comparisons from `r nEC_MR` independent studies.
...
p1 + plot_spacer() + plot_spacer()  + p6 + p5 + p3 + plot_layout(design = layout)
}
...
```{r model 3: +particle material, eval=FALSE, include=FALSE}
morethan1particlematerial <- length(unique(df$ParticleMaterial))>1
print(c('more than 1 particle material: ',morethan1particlematerial))

if (morethan1particlematerial)  {
df_MR <- select(df, c("Year", "StudyID","Study", "EScluster", "ESid","DevelopmentalStage",
...
if (n_outcome!=10){
anova(mMR[[mname2]], btt=c("DevelopmentalStage","ExposureDurationDays","ParticleSizeMean","ParticleMaterial"))
}else if (n_outcome==10){ #for Sensory-motor function
  anova(mMR[[mname2]], btt=c("ExposureDurationDays","ParticleSizeMean","ParticleMaterial"))
}
m<-mMR[[mname2]]
...
p1 + plot_spacer() + plot_spacer()  + p7 + p5 + p3 + plot_layout(design = layout)
}
