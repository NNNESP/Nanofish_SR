...
a) Study + Experiment ID does not fit the data significantly better than
the only Experiment ID model for GSH content, Motor function: Distance,
Feeding time and Speed during feeding

b) Study + Experiment ID does not fit the data significantly better than
the only Study model for almost all variables, except for AChE activity,
GPx activity and Sensory-motor function: Distance

c) Study + Experiment ID does not fit the data significantly better than
the only Research Cluster model for half of the variables.
...
Additionally, inclusion of ES cluster for variables feeding time and
speed during feeding resulted in over-fitting.
...
m1 = vector("list",4)
names(m1) <- c("null","forest","random_effects","rob_random_effects")
m1$null =        vector("list",nlevels(outcomeslist))
m1$forest =        vector("list",nlevels(outcomeslist))
m1$random_effects =        vector("list",nlevels(outcomeslist))
m1$rob_random_effects =        vector("list",nlevels(outcomeslist))

rows = c("k estimates","N studies","ES","SE","ci.lb","ci.ub","pi.lb","pi.ub","I² total","Q","Q p-value")
predicted_intervals = data.frame(matrix(nrow=length(rows), ncol=length(outcomeslist))) colnames(predicted_intervals) = outcomeslist
...
for(outcome in outcomeslist)
{
 ...
 # Heterogeneity
 heterogeneity[[outcome]]= round(i2_ml(m1$rob_random_effects[[outcome]]),2)
...
{ 
  pdf(file=paste('Figures/Orchard',str_replace(outcome,':','_'),'REmodel','.pdf'),width=4,height=6)
  
  orchardRE_model(m1$rob_random_effects[[outcome]],outcome,I_yposition,heterogeneity)
...
Level 1 - Sampling Level 2 - Effect size cluster: within studies
variability
Le![](http://127.0.0.1:42065/chunk_output/s/2D748050/c7x2tw09ur7gj/000030.png)vel
3 - Study cluster: between studies variability orchard plot displays the
prediction interval of the overall effect (bold whiskers), the number of
effect sizes (k) and the number of studies (the number in the bracket).
