...
bubble_plot <- function(object, mod, group = NULL, xlab = "Moderator", ylab = "Effect size", N = "none",
                        ...
                        weights = "prop", by = NULL, at = NULL, cond_levels = NULL,colorby=NULL,colormap=NULL,fill=NULL)
{
  ...
  if(is.null(data_trim$condition) == TRUE){
    plot <-ggplot2::ggplot() +
      # putting bubbles
      ggplot2::geom_point(data = data_trim, ggplot2::aes(x = moderator, y = yi, size = scale, fill = colorby, color=colorby), alpha = alpha ) +
