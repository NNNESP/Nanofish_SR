...
```{r global options, echo=TRUE, warning=FALSE, cache=FALSE}
library(knitr)
library(rmdformats)
...
```{r}
p1<-funnel(df$yi, df$vi, yaxis="sei",
       #xlim = c(-3, 3),
       xlab = "Effect size (Zr)", digits = 2, las = 1) 
p2<-funnel(df$yi, df$vi, yaxis="seinv",
       #xlim = c(-3, 3),
       xlab = "Effect size (Zr)",  digits = 2, las = 1) 
p1+p2
```
...
m <- mRE
 t3 <- data.frame("n studies"     = sapply(m, function(x) x$s.nlevels[1]),
                  ...
                  "σ²total"       = sapply(m, function(x) x$sigma2[[1]]),
                  "σ²Study"       = sapply(m, function(x) x$sigma2[[2]]),
                  "σ²EScluster"   = sapply(m, function(x) x$sigma2[[3]]),
                  "σ²ES"          = sapply(m, function(x) x$sigma2[[4]]))
 ...
 # colnames(t6) <- c("Overall effect (pooled SMD)", "Standard error", "p-value", "Lower CI", "Upper CI","Lower PI","Upper PI")
  ...
  t4<-data.frame(sapply(mRF,  fitstats))
rownames(t4) <-rownames(fitstats(mRF[[1]]))
DT::datatable(t4)
```
...
```{r echo=TRUE}
influentialcases_plots(mRE,"ML_RobVar"),"ML",0.01)
