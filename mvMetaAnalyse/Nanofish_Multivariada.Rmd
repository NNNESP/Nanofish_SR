...
```{r echo=TRUE}
colormap <-c("AChE activity" = "#ff65acff", 
            ...
            "Motor function: Distance" = "#24b700ff",
            "Sensory-motor function: Distance" = "#8cab00ff",
...
p[[n]]<-orchard_plot(m, mod = "1", xlab = "Effect size", group = "Study", k = TRUE, g = TRUE, transfm = "none", angle = 0,legend.pos = "top.out") + 
  ...
  scale_fill_manual(values = colormap[[n]]) +
  scale_colour_manual(values = colormap[[n]])+
...
## Models
OBS: Results are not appropriate because it is not possible to include multilevel structure
...
VCV <- vcalc(vi, cluster = interaction(df_original$EScluster,df_original$Study,df_original$Outcome), obs = ESid, data = df_original, rho = VCVrho)
```
...
MV[[3]] <- rma.mv(yi, VCV,random = ~ Outcome | Study, mods= ~  0 + Outcome +
                 Outcome:(ParticleSizeMean + Concentration_mg_L + ExposureDurationDays*DevelopmentalStage),
                        data=df_original,slab = Study, method="REML",test="t",
                        dfs="contain", level=95) #it will only keep complete cases
#Aviso: 84 rows with NAs omitted from model fitting.Aviso: Redundant predictors dropped from the model.Aviso: One or more levels of inner factor (i.e., Feeding time) removed due to NAs.

df_original$Year_centered <- df_original$Year-mean(df_original$Year)

MV[[4]] <- rma.mv(yi, VCV,random = ~ Outcome | Study, mods= ~  0 + Outcome +
                 Outcome:(ParticleSizeMean + Concentration_mg_L + ExposureDurationDays*DevelopmentalStage + Year_centered + vi),
                        data=df_original,slab = Study, method="REML",test="t",
                        dfs="contain", level=95) #it will only keep complete cases
#Aviso: 84 rows with NAs omitted from model fitting.Aviso: Redundant predictors dropped from the model.Aviso: One or more levels of inner factor (i.e., Feeding time) removed due to NAs.
...
summary(MV[[2]])
print("Multivariate of outcomes with moderators")
summary(MV[[3]])
print("Multivariate of outcomes with moderators and publication bias")
summary(MV[[4]])
```
